...
쿠키는 세션 쿠키(session cookie) 와 지속 쿠키(persistent cookie) 두가지 타입으로 나눌 수 있습니다.

- 세션 쿠키 - 사용자가 사이트를 탐색할 때 관련한 설정과 신호 사항들을 저장하는 임시 쿠키

* 지속 쿠키 - 지속 쿠키는 삭제되지 않고 더 길게 유지되며, 디스크에 저장되어 브라우저를 닫거나 컴퓨터를 재시작해도 남아있습니다. <br>지속 쿠키는 사용자가 주기적으로 방문하는 사이트에 대한 설정 정보나 로그인 이름을 유지하려고 사용합니다.

두 쿠키의 차이점은, 파기되는 시점 뿐 입니다. Expires 또는 Max-Age 파라미터가 없으면 세션 쿠키가 됩니다.

#### 쿠키의 동작원리

- 처음에 사용자가 웹 사이트에 방문하면 웹 서버는 사용자에 대해서 아무것도 모릅니다.
- 웹 서버는 사용자가 다시 돌아왔을 때, 해당 사용자를 식별하기 위한 유일한 값을 쿠키에 할당합니다. HTTP 응답 헤더에 기술되어 사용자에 전달합니다.
