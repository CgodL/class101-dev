...
컬럼은 테이블을 구성하는 데이터의 속성(attribute)을 말합니다. 이런 컬럼들이 모여 하나의 테이블을 구성합니다. 반면 로우는 지정된 컬럼 데이터로 이루어진 하나의 묶음입니다. 따라서 컬럼별로는 이름, 성별처럼 그 성격이 다르지만, 로우별로는 ‘이름,성별’이 한 묶음이 되어 성격이 다르지 않습니다.
