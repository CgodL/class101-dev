...
데이터를 의미있는 값만 뽑아내어 CSV 파일로 변환하여 데이터를 관리하겠습니다. 아니면 DB안에 담아야 할까요..<br>
CSV 파일로 만들기 위해서는 컬럼이 필요합니다. 컬럼에 의미 있는 값을 주고 이에 맞춰 분류를 해야 할 것 같은데..
예를 들면서 구상해봐야할 것 같습니다.

예를 들어, 날씨 라는 컬럼을 만들고 여기에 들어갈 데이터에는 태그안에 날씨가 포함되어 있거나,

> 데이터 가공을 목표로 하는 프로젝트에 맞춰서 해야할 것 같습니다.
