...
**리팩토링 / 모듈화**<br>
리팩토링 하며 반복되는 부분에 대해서 모듈파일을 따로 만들어, export 해줍니다. 저 같은 경우 ES6를 위한 바벨 설정을 따로 해주지 않았으므로 commonJS를 활용하여, 모듈화 를 했습니다.
로직은 간단합니다. 반복이 필요한 부분에 있어서 함수로 만들어 따로 모듈폴더에 파일을 생성하고, 이를 exports 하고 require하여 사용합니다.
...
> 자연어 처리 이전에 하드코딩으로도 처리할 수 있습니다.
> 로직은 단순하게 except 변수를 만들어, except에 해당하는 데이터를 includes 하는 것을 예외 처리해버리면 됩니다.
...
> <b>-</b>ES6를 이용하지 않고 commonJS로 모듈을 exports 할때의 불편함

> <b>-</b>**CSV 변환문제** : 현재 코드는 데이터를 크롤링하고, 이를 바로 CSV로 변환하는데, 이후 append할 경우, 기존 CSV 데이터의 수치가 증감 하는 형태가 아닌, 이전 데이터 그대로 다시 추가되는 의미없는 반복이 지속 되는 문제가 있습니다. 이는 제가 고려했을때 DB에 데이터를 저장하고, 클라이언트 단에서 get요청을 보낼 때에, CSV를 생성하도록 하면, 데이터는 계속 추가되고, 종합적인 데이터가 CSV로 저장되어 사용되지 않을까 .. ( 이름을 랜덤으로 부여해서 특정 시간대에 이 CSV를 쓴다 식으로 구현 한다면 ?)
